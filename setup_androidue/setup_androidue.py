...
class SetupUtils:
    ...
    def set_usb_tethering_ip(self, ip, serial):
        if not('rndis0' in self.run_adb_shell_cmd('ip link show', True, serial)):
            ...
            return
        # Add respective IP address in the UE
        self.run_adb_shell_cmd('ip address add ' + ip + '/24 dev rndis0', True, serial)
        # Configure UEs routing table
        self.run_adb_shell_cmd('ip route add 192.168.42.0/24 dev rndis0 table local_network', True, serial)

    def run_dropbear_server_instance(self, port, pub_key=None, serial=None):
        # Kill all active Dropbear instances before starting a new one
...
if __name__ == "__main__":
    ...
    for serial in serials_l:
        if 'unauthorized' in serial:
            print('\nWARNING: Device with serial ' + serial.replace('unauthorized', '').strip() + ' is not authorized, '
                  'check if USB-debugging is enabled in the developer settings of your phone.\n' 
                  'If it is activated, unlock your phone and you should see a popup where you can authorize the new USB-debugging connection.\n')
            input('Press Enter if you have authorized the device:')
    ...
    while True:
        ...
        if ue_serial == 'exit':
            ...
            print("\nWARNING: Serial %s not available! Serials of connected devices:" % ue_serial)
            print(serials_l)
            print("\n\n")
            continue
    ...
    if 'yes' in input('Do you want to set up IPTables on OGT Secondary Unit? (yes/no): '):
        ...
        while ssh_intf_name not in netifaces.interfaces():
            ssh_intf_name = input('Interface not found, enter an existing interface: ')
        if 'yes' in input('Do you want to delete a previously created bridge if there is any? (yes/no)'):
            subprocess.Popen(['sudo', 'ifconfig', 'ogt', 'down'])
